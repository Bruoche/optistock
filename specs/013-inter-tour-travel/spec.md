...
- **A connecting drive's duration is unknown**: If a travel leg (warehouse↔stop or stop↔stop) cannot be routed, its duration is **unknown** and the leg contributes **0** to a **best-effort** projected day, which is then **flagged as approximate/incomplete** (FR-015) so the manager sees a usable lower bound and knows extra point-to-point time may be missing. The routing failure is logged. (The per-leg unknown state is still kept distinct from a genuine zero at the leg level — FR-010.)
...
- **FR-009**: When a connecting travel leg's duration cannot be determined (a routing error), the system MUST log the failure and MUST treat that leg as **0 in a best-effort projected day** rather than hiding the whole figure. It MUST NOT silently present the reduced total as exact — see FR-015. (This supersedes the feature-012 approach of propagating unknown to the whole projected figure; the per-leg unknown/zero distinction of FR-010 still holds at the leg level.)
...
- **FR-012**: The start/end stop selection and the resulting travel legs MUST be recorded with the tour/assignment so the projected day is reproducible and consistent with what was shown at assignment time.
...
- **Projected day accuracy flag**: a per-driver marker that is set when at least one leg feeding that driver's projected figure failed to route, telling the manager the figure is a lower bound that may understate point-to-point travel.
...
- **SC-005**: Whenever any leg feeding a driver's projected day fails to route, that driver's figure is shown as a best-effort lower bound **and** carries the approximate/incomplete flag — in 100% of such cases; a driver with all legs routed is never flagged.
- **SC-006**: No distinct travel leg is requested from the routing API more than once per driver-list load, and concurrent in-flight leg requests never exceed the configured cap (verifiable by counting the faked routing calls and the peak concurrency in a test).
