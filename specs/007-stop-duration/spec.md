...
- **Invalid input handling**: Empty, non-numeric, or negative entries fall back to a valid non-negative value (treated as 0 or reset to the default); the chosen fallback must never let the total become NaN or negative. Final pick to be confirmed at planning.
...
- **No upper bound** on a single stop's duration is enforced beyond sane integer handling; very large values must still total and format correctly.
