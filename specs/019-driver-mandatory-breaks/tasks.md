...
- [X] T012 [P] [US2] Extend `resources/js/hooks/use-tour-drivers.test.ts`: a payload with `added_break` maps onto `Driver.addedBreak` (incl. a non-zero value).
- [X] T013 [US2] Extend `resources/js/components/tour/driver-list.test.tsx`: a driver with `addedBreak > 0` renders a "Required break" figure reading `+…` in the orange role, positioned left of "To tour"; a driver with `addedBreak === 0` renders no such figure.
...
- [X] T014 Run the full gate: `npm run format:check`, `npm run lint:check`, `npm run types:check`, `npm run test`, `./vendor/bin/pint --dirty --test`, `php artisan test` — all green.
