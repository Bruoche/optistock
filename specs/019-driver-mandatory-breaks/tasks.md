...
- [ ] T011 [US2] In `resources/js/components/tour/driver-list.tsx`, render a "Required break" figure as the leftmost child of the right-hand figure group, only when `driver.addedBreak > 0`: value `+${formatDurationHm(driver.addedBreak)}` in the `--primary` (orange) emphasis role (`text-primary`), muted uppercase label like the others. No change to the other figures.
