...
`services.openstreet.route_precision` (Google polyline precision; 6 — verified live),
...
## 4. Verified live (2026-06-07)

Both former unknowns are confirmed: polyline **precision 6** (`OPENSTREET_ROUTE_PRECISION`) overlays real
roads; `total_time` is **seconds** (sound durations: ≈20 min city, ≈1 h Paris, ≈26 h cross-country). If a
future API change shifts the precision, it's a one-value config tweak — no code change.
